...
<?php if ($activity_ && count($activity_["definitions"]) > 3) : ?>
    <form action="controller.php" method="POST">
        ...
        <input type="hidden" name="activity" value="Unscramble the Definition">
        ...
        <h1 style="margin-top:0;text-align:center">Unscramble the Definition</h1>
        <fieldset class="grey">
        ...
        <fieldset class="grey">
            ...
            <ol>
                <h3>Instructions</h3>
                <li>Look at the image and defintion.</li>
                <li>Unscramble the Defintion.</li>
            </ol>
            <div class="items">
                <?php foreach ($activity_["definitions"] as $i => $definition) : ?>
                    <input class="input" type="hidden" name="target[]" value="<?= $definition["data"]["definition"] ?>">
                    ...
                    <div class="item">
                        ...
                        <div class="answer">
                            <?php if (isset($_SESSION["answer"])) : ?>
                                <textarea readonly cols="40" rows="3" class="input" style="display:block;margin:5px auto;text-align:center;font-size:16px" type="text"><?= $_SESSION["answer"][$i] ?></textarea>
                            <?php else : ?>
                                <textarea readonly cols="40" rows="3" class="input" style="display:block;margin:5px auto;text-align:center;font-size:16px" type="text" name="input[]" required></textarea>
                            <?php endif ?>
                        </div>
                        <?php if (!isset($_SESSION["answer"])) : ?>
                            <div style="text-align:center;padding:10px" class="keyboard">
                                <?php foreach ($definition["data"]["definition_scramble"] as $word_scramble) : ?>
            ...
            <div class="buttons">
                ...
                <div style="margin-bottom:20px">
                    <a style="float:right" href="?page=Type the Word&id=<?= $id ?>">Next Activity</a>
                </div>
            </div>
        </fieldset>
    </form>
