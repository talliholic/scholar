...
<?php if ($activity && count($activity["definitions"]) > 3) : ?>
    <form>
        ...
        <input type="hidden" name="activity" value="Choose Image from Definition">
        ...
        <h1 style="margin-top:0;text-align:center">Choose Image from Definition</h1>
        <fieldset class="grey best_results" id="best_results"></fieldset>
        <fieldset class="grey">
            <legend><?= $activity["title"] ?></legend>
            <ol>
                <h3>Instructions</h3>
                <li>Read the definition out loud.</li>
                <li>Select the image that BEST describes it.</li>
            </ol>
            <div class="items">
                <?php foreach ($activity["definitions"] as $i => $definition) : ?>
                    <div class="item">
                        <div class="question">
                            <div style="display:flex;flex-flow:row wrap;column-gap:10px">
                                <div class="word" id="<?= $definition["data"]["img_path"] ?>"><?= $definition["data"]["definition"] ?>
                                </div>
                        ...
                        <div class="answers">
                            <?php foreach ($definition["options"] as $option) : ?>
                                <div class="option">
                                    ...
                                    <input class="input radio" type="radio" name="question_<?= $i ?>" value="<?= $option["img_path"] ?>" required>
                                    <img class="medium-img" src="<?= $option["img_path"] ?>" alt="<?= $option["word"] ?>">
                                </div>
                            <?php endforeach ?>
                        </div>
                    </div>
            ...
            <div class="buttons">
                ...
                <div style="margin-bottom:20px">
                    <!-- <a href="/scholar/stats/?page=best_by_activity&activity=Choose Image From Definition&guide_id=<?= $id ?>">See best results</a> -->
                    <a style="float:right" href="?page=Choose Word from Image&id=<?= $id ?>">Next Activity</a>
                </div>
